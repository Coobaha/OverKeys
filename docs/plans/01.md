...
# Prompt 1: Add macOS releases to GitHub Actions 🍎

Create a GitHub Actions workflow that builds and releases macOS versions of OverKeys alongside existing Windows releases.

CRITICAL CONTEXT: OverKeys is being ported to macOS (75% complete). Need automated macOS builds/releases in CI.
GOAL: Extend existing release workflows to include macOS platform support.

- [x] Create macOS build workflow that triggers on releases
  - [x] Configure macOS runner (macos-latest)
  - [x] Setup Flutter environment for macOS builds
  - [x] Build macOS app bundle (.app)
  - [x] Create DMG installer for distribution
  - [x] Upload macOS artifacts to GitHub releases
- [x] Update existing release-please.yml to handle macOS releases
- [x] Ensure proper artifact naming for platform differentiation
- [x] Test workflow with sample release
